...
last_verified_against: SolutionSmith-debug/its@e242074 (documentation-reconciliation pass)
supersedes: workstreams/progress-reporting/mission.md@v1
...
# ITS — Progress Reporting — Mission v2

**2026-07-12 — v2 (canonical): promoted `draft` → `canonical` — the load-bearing pipeline is ratified-and-live (documentation-reconciliation pass; exec `SolutionSmith-debug/its@e242074`).** The two conditions the v1 draft named as its promotion gate are **both met**. **(1) Doctrine ratified.** The §50/§51 gates were **ratified by the Developer-Operator on 2026-06-29** as Op Stds v19 §50 (privileged code-actuation gate) and §51 (ITS-owned structured-SoR write-back), now carried forward in **Op Stds v20** — so the §16 flags are resolved (see the in-section resolution banner) and P7 + M2/M3 are doctrine-unblocked. **(2) Pipeline landed + live.** The Progress-Reporting send/compile pipeline is **built and live** — the deterministic `progress_weekly_generate` (instantiating the hardened shared compile core), the two-process AI-free `progress_send` + `progress_send_poll` sender, `wpr_review` / `WPR_human_review`, and the field-ops mirror suite (`hours_log` / `equipment_status` / `material_list` / `material_incidents`) all present in `progress_reports/` — with **P4/P5/P7 + M3 (mirror suite) reported FULLY LIVE per the 2026-07-09 field-ops state**. This v2 is a **status flip + stale-preamble correction**, not a rewrite: the design body below (§§1–17) is the **unchanged design source**; only the frontmatter status, this promotion note, the §16 resolution banner (already present), and the §17 authority framing reflect the ratified-and-live reality.

**2026-06-28 — v1 (draft, retained for provenance): codifies the just-locked "ITS — Progress Reporting" + P3 Materials program into the planning layer.** This mission is the architecture/decision home for the Progress-Reporting build: the **structural twin of the Safety Portal weekly pipeline** — an **ITS-owned Smartsheet structured system-of-record + Box document system-of-record + a Weekly Progress Report that is human-reviewed and *sent externally* to the job stakeholder** — plus **P3 Materials** (an admin type catalog + a per-job material-list/manifest receive flow). It is authored from the operator-confirmed master plan (`~/.claude/plans/let-s-go-with-option-greedy-fiddle.md`, "ITS — Progress Reporting + P3 Materials, scaling-reconciled"), the execution handoff brief (`~/its/docs/cc-brief_progress-reporting-program_2026-06-28.md`), and the 2026-06-28 execution session (exec `SolutionSmith-debug/its@9ef3d5b`; [memory-archive §G43](../../references/memory-archive.md)). The program was hardened through grill-me + four reviewers + a contamination hunt + a four-fork ultra-plan pass + a reconciliation against the **2026-06-28 forensic scaling eval (20×20)**, with five strategic decisions locked. _(Superseded by v2 — provenance:_ at v1 authorship the status was `draft`, not `canonical`, because the load-bearing pipeline (Stage-1 parameterize, Stage-2 progress send/compile, the SoR up-sync, the Material List receive) was then a forward plan gated on **then-unratified doctrine** (the §50/§51 SoR-write decision, Seth-only — §16), with only the Tier-A foundation (M1/P-A1/A2 + the field-ops UI fix) landed. **Both gates are now met** — doctrine was ratified 2026-06-29 and the pipeline is built + live — so **v2 promotes this mission to `canonical`**; see the v2 note above and the [Authority](#17-authority) section.)_
...
ITS — Progress Reporting Mission **v2, canonical, 2026-07-12** (promoted from v1 draft, 2026-06-28). Authored from the operator-confirmed master plan (`~/.claude/plans/let-s-go-with-option-greedy-fiddle.md`) + the execution handoff brief + the 2026-06-28 execution session (exec `SolutionSmith-debug/its@9ef3d5b`; [memory-archive §G43](../../references/memory-archive.md)). Both Foundation invariants are restated verbatim from [Foundation Mission v11](../../doctrine/foundation-mission.md) (§8). Workstream missions are frontmatter-versioned (not git-tagged).

**Now `canonical` (v2).** The locked strategic decisions (§9) remain in force and non-negotiable. The canonical-promotion trigger the v1 draft named — the Stage-2 progress send (P5) lands **and** the §50/§51 SoR-write doctrine is ratified — is **met**: §50/§51 were ratified 2026-06-29 (Op Stds v19, carried forward in v20; see the §16 resolution banner), and the send/compile pipeline (`progress_weekly_generate` → `WPR_human_review` → `progress_send` / `progress_send_poll`) plus the field-ops mirror suite (`hours_log` / `equipment_status` / `material_list` / `material_incidents`) are built + live (P4/P5/P7 + M3 mirror suite reported FULLY LIVE per the 2026-07-09 field-ops state). This mission therefore reconciles to `canonical` against exec `SolutionSmith-debug/its@e242074`, with the §16 flags resolved. Remaining forward work — the optional `weekly-progress-summary` form, the UI follow-ups, and Personnel-CRUD's three open product decisions (§13) — is slice-overlay progress absorbed without a version bump, not a draft-gating blocker.
...
v3 trigger: a substantive scope change — the canonical-Evergreen Smartsheet integration moving in scope (PJOB→JOB reconciliation), a swap of the ITS-owned-SoR posture, or a Personnel-CRUD product decision that reshapes the field-ops surface this depends on. (The v2 trigger — doctrine ratification of the §50/§51 flags plus the Stage-2 pipeline landing + going live — is now consumed by this promotion.) Status-overlay updates absorb slice-landing / activation progress without a major bump.
