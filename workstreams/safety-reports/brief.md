...
The weekly product is a **deterministic merge** (`form_pdf.merge_pdfs`) of the week's submitted-form PDFs **+ a fixed-template email body — no Anthropic / LLM call** (the prior Anthropic-drafted-narrative WPR is retired with the portal cutover; LLM stays in scope for other workstreams only). Two-process model per FM v8 Invariant 1 holds structurally: the compile process has **zero send capability** and writes the draft row to `WSR_human_review` (the standalone ITS — Safety Portal workspace; supersedes `WPR_Pending_Review` under the clean-break) with the **template-seeded**, editable Email Body as the send's source of truth. Friday-afternoon launchd cadence (or the `Compile Now` checkbox). The compiled packet is uploaded to the week's Box folder with **version-on-conflict** (`box_client.upload_bytes_or_new_version`, PR-G #186) — a recompile re-produces the same deterministic packet name → a new Box VERSION rather than a 409 that breaks Compile-Now; and the packet is attached inline on the Rollup + `WSR_human_review` rows (PR-F #184). The week folder is the **config-gated Box mirror tree** (PR-K #189): when `ITS_Config` `safety_reports.box.portal_root_folder_id` is set, `weekly_generate` files into `root → per-job → per-week` via the shared `safety_naming` names (mirroring Smartsheet); unset → the legacy per-job category layout. **Code-state (exec `f3ad814`):** deterministic rewire + version-on-conflict + inline-attach + the Box-mirror branch all LANDED; `portal_poll.py` is a live launchd daemon (PR-A #179). Mirror-account validation deploy in flight; the Box mirror is live-inert until the operator sets the root key (see Safety Portal brief §9/§11).

## Weekly send (safety_reports/weekly_send.py) — CODE-COMPLETE (PR #176, `e628044`)

Reads approved rows from `WSR_human_review` (ITS — Safety Portal workspace) where `Approve for Scheduled Send`=true, reading the human-edited Email Body as the source of truth. Sends via Resend or Graph send_mail. Zero AI step per FM v8 Invariant 1. **F22 approval authority** is the Safety Portal **workspace membership** (`weekly_send_poll._load_authorized_approvers` → `list_workspace_share_emails`; the `authorized_approvers` allowlist retired — PR-E #183, see [Safety Portal mission §8.1](../safety-portal/mission.md#81-f22-approval-authority-is-realized-as-workspace-membership-pr-e-183)). **Recipients are resolved at send time** from `ITS_Active_Jobs` (TO = Safety Reports Contact, CC 1–5); an unresolvable job or empty/invalid TO sets **Send Status = HELD** with a WARN surface and is excluded from poller re-dispatch — **surface-not-strand**, never a silent send.

## Email-intake path — preserved for Email Triage (retain rationale, 2026-06-07)

The clean break retired **only** the safety email *input*, not the email *infrastructure*. The following stay **resident in `main`** and **must not be decommissioned** as a "clean-break cleanup" — they are the seed for the committed [Email Triage](../email-triage/mission.md) workstream (the Graph plumbing is workstream-agnostic):

- **Preserved-dormant** (no live caller; reachable, parses, not tombstoned): `week_folder.py` (the Monday-ISO Field-Reports folder scaffold + its `FIELD_REPORTS_FOLDER_BY_PROJECT` consumer), `intake.process_message` + the Graph-fetch / classify / extract stages, `shared/graph_client.py` / `untrusted_content.py` / `header_forgery.py`.
- **Actively LIVE shared infra** (reused by the *portal* Box path today, so doubly load-bearing): `shared/project_routing.get_folder_id` + `defaults.BOX_PROJECT_FOLDERS` (the Box-folder routing, with its deliberate hardcoded fallback — distinct from `active_jobs`, which has none) and the report-**category** machinery (`BOX_SUBPATH_BY_CATEGORY` / `VALID_CATEGORIES`, reused via `PORTAL_FORM_CATEGORY`).
- **Tombstoned (the only one):** `intake_poll.py` (`NotImplementedError`, PR #171) — the email *poller*, superseded by `portal_poll.py` for the safety path.

This is the recorded reason these modules stay; see memory-archive §G26 and the [Email Triage mission](../email-triage/mission.md) forward-reference.
...
Safety Reports Brief **v6.5**, 2026-06-07 canonical (verified against exec `f3ad814`). Status overlays since v6.1 (2026-05-22): v6.2 WSR/clean-break, v6.3 LLM-free, v6.4 WSR-rewire code-complete, v6.5 deploy-session (Box version-on-conflict + email-path retain). Original v6.1 absorbed R3 Session 1 ship + polling daemon + heartbeat + Mail.app trigger retirement + Mission v5.1 pointer. Workstream briefs are frontmatter-versioned (not git-tagged).

v7 trigger criteria: substantive engineering architecture change (e.g., new module decomposition pattern, fundamentally different trigger mechanism). v6.x absorbs ship-status updates without architecture changes (the v6.2–v6.5 overlays are all within-scope deploy/clean-break status, not architecture changes).