...
**Last refreshed:** 2026-06-09 (afternoon — publish-pipeline hardening + WSR timestamps. PRs #236/#241/#242/#244/#245 merged to main (`2aa2061`). Publish pipeline fully end-to-end: blank-PDF archive keyed by definition id (#236), daemon uses `sys.executable` not bare `python` (#241), Retire stepper labels corrected (#242), redundant-retire now fails cleanly (#244), WSR Approved At / Sent At now carry date+TIME as ABSTRACT_DATETIME Pacific (#245). Smartsheet live finding: ABSTRACT_DATETIME accepts naive `YYYY-MM-DDTHH:MM:SS`; rejects offsets/Z. `~/its` now on `main` (`2aa2061`). Publish daemon IS loaded (`org.solutionsmith.its.publish-daemon` live). Next: edge-case matrix, Evergreen production cutover.)
...
- **Safety Portal Phase-2 Form Manager — BUILT (2026-06-09, PRs #203–#216 + #218) + publish-pipeline hardened (2026-06-09 afternoon, PRs #236/#241/#242/#244/#245).** See "Recently landed" above + memory-archive §G30 + §G31. Deferred: rollback UI picker (frontend only — backend done), S1 per-item scale/comment authoring, integration test harness for the daemon's git/gh/wrangler subprocess chain, portal admin Retire UI gate (shows for already-retired forms; backend rejects clean), `README.md:111` doc-drift (weekly-send idempotency key), `form_archive_out/` temp-dir cleanup.
- **`~/its` is on `main` (`2aa2061`)** — resolved. Publish daemon loaded + healthy.
...
- **WSR_human_review live columns retyped DATE → ABSTRACT_DATETIME** (mirror, 2026-06-09) — `2aa2061` code writes naive Pacific timestamps; PRODUCTION sheet built from updated builder gets ABSTRACT_DATETIME from creation.

### On the horizon
- **Safety Portal** — **Phase-2 Form Manager FULLY BUILT + publish pipeline hardened (2026-06-09, exec HEAD `2aa2061`).** Phase-1 mirror-activated + live-validated (2026-06-08). Publish daemon IS loaded (live as of 2026-06-09). `portal_poll` + `weekly_send_poll` live + healthy. `ZZ Portal Proof` (JOB-000008) Active. Next: (1) full edge-case matrix (bad-HMAC, dup-UUID, malformed payload, inactive-job, amend-supersede, empty-week, Friday-skip, empty-TO HELD, F22 non-member); (2) load `compile_now_poll` daemon; (3) activate Orphaned Reports (flip `SHEET_ORPHANED_REPORTS`); (4) Evergreen production cutover. Deferred form-editor items: rollback UI picker (frontend only), S1 per-item scale/comment. See memory-archive §G27 (activation) + §G30 (Phase-2 build) + §G31 (pipeline hardening + WSR timestamps).
