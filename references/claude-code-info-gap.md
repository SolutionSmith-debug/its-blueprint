...
**Last refreshed:** 2026-06-09 (Safety Portal Phase-2 Form Manager fully built — PRs #203–#216 + #218 merged to main (`b736691`); catalog manifest contract + `apply_publish` core + B8 form editor UI + publish pipeline (Worker send-free enqueue, `publishValidation.ts`, daemon queue endpoints, privileged Mac actuator `publish_daemon.py` w/ `_reset_to_main`) + 3-renderer render-smoke net + session hardening (8a `session_epoch` real revocation, 8b admin 5-min idle). GitHub auto-merge DISABLED — daemon polls `mergeStateStatus` then `gh pr merge --squash` directly. New ITS_Config row: `safety_reports.publish_daemon.polling_enabled = true`. Launchd `org.solutionsmith.its.publish-daemon` on disk, NOT loaded (operator-gated; Tier-3 HIGH-capability). `~/its` currently on `publish/req-5-incident-report` branch — `_reset_to_main` will recover on next daemon cycle or manual `git checkout main`. Next: load publish daemon, edge-case matrix, Evergreen production cutover.)
...
- **Safety Portal Phase-2 Form Manager — BUILT (2026-06-09, PRs #203–#216 + #218).** See "Recently landed" above + memory-archive §G30. Deferred: rollback UI picker, S1 per-item scale/comment authoring, integration test harness for the daemon's git/gh/wrangler subprocess chain.
- **`~/its` stranded on `publish/req-5-incident-report` branch** — next daemon cycle (once `publish_daemon.py` launchd job is loaded) will `_reset_to_main` automatically; or run `git -C ~/its checkout main && git pull` manually.
- **Publish daemon NOT yet loaded** — launchd plist on disk (`org.solutionsmith.its.publish-daemon`), RunAtLoad false; operator gates due to HIGH-capability Tier-3 class (commits code, deploys).

### On the horizon
- **Safety Portal** — **Phase-2 Form Manager FULLY BUILT (2026-06-09, exec HEAD `b736691`).** Phase-1 mirror-activated + live-validated (2026-06-08). Next: (1) load `publish_daemon` launchd job (operator-gated, Tier-3); (2) full edge-case matrix (bad-HMAC, dup-UUID, malformed payload, inactive-job, amend-supersede, empty-week, Friday-skip, empty-TO HELD, F22 non-member); (3) Evergreen production cutover. Deferred form-editor items: rollback UI picker, S1 per-item scale/comment. See memory-archive §G27 (activation) + §G30 (Phase-2 build). `portal_poll` + `weekly_send_poll` live + healthy. `ZZ Portal Proof` (JOB-000008) Active.
