...
**Last refreshed:** 2026-06-10 (PR #258 — admin idle window 5→30 min + bounded dirty-editor keep-alive. `useIdleLogout` gained a `paused` arg; `AdminApp` threads `editing` flag from `FormsPage`/`AccountsPage`; keep-alive fires `fetch /api/session` every 240s while paused, but proactive logout still fires in both modes so an abandoned dirty tab dies at ≈30 min. Unmount-reset test gap closed (`FormsPage.editing.test.tsx` + `useIdleLogout.test.ts`). Deployed Worker version `276322a3`. `~/its` on `main` (`23c04e6`). All prior carried open items (M1/M2/M4/M5/M6/M7/M9 + ITS_Daemon_Health + CLAUDE.md v16 stale) remain open.)
...
### On the horizon
- **Safety Portal** — **Phase-2 Form Manager FULLY BUILT + pipeline hardened + UX polished + forensic audit filed + admin idle-timeout hardened (2026-06-10, exec HEAD `23c04e6`; mirror SPA live, Worker version `276322a3`).** Phase-1 mirror-activated + live-validated (2026-06-08). Publish daemon + portal_poll + weekly_send_poll all live + healthy. `incident-report` v1/v2 in catalog. Admin idle window 30 min; bounded dirty-editor keep-alive in place. `ZZ Portal Proof` (JOB-000008) Active. Next: (1) full edge-case matrix (bad-HMAC, dup-UUID, malformed payload, inactive-job, amend-supersede, empty-week, Friday-skip, empty-TO HELD, F22 non-member); (2) load `compile_now_poll` daemon; (3) activate Orphaned Reports; (4) resolve CLAUDE.md v16→v18 doctrine drift (M9); (5) resolve ITS_Daemon_Health drift; (6) one-time Box archive backfill for reqs 11/12/13; (7) Evergreen production cutover. See memory-archive §G27 (activation) + §G30 (Phase-2 build) + §G31 (pipeline hardening) + §G32 (UX) + §G33 (forensic audit + E2E validation) + §G34 (idle-timeout hardening).
