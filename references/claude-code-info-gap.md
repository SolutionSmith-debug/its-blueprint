...
**Last refreshed:** 2026-06-13 (exec PR #283 landed: `fix(safety-portal)` — 50-char Smartsheet sheet-name cap enforcement in `week_sheet.py` + new `SmartsheetValidationError` typed subclass + portal-submission permanent-400 drain to Review Queue + §43 runbook row. Root-caused by JOB-000013 long project name. Prior (2026-06-12): exec PRs #271–#276 landed + blueprint mission v4→v5 reconciliation — see §G36/§G37.)
...
- **`~/its` is on `main` (`e75c5a7`)** — publish daemon loaded + healthy.
