...
**Last refreshed:** 2026-06-28 (Field-Ops write-UI phase complete — PRs #319–#322 four-part verified; P2.4 SoR mirror daemon BLOCKED (no canonical Smartsheet access). See §G42.)
...
### Open queue
- **Field-Ops P2.4 SoR mirror daemon — BLOCKED (2026-06-28).** Operator has no access to the canonical Evergreen Smartsheet; the real schema/SoR is unseen. Parked until SoR visibility granted. Unblock condition: Seth gets read access to the live Evergreen Smartsheet so the schema can be observed before building. Design constraints are captured in `docs/cc-brief_fieldops-next-session.md` (PR #323, pending merge).
- **PR #323 pending merge** (branch `docs/fieldops-session-close-2026-06-28`, in CI as of 2026-06-28): session log `docs/session_logs/2026-06-28_field-ops-write-ui-phase.md` + `docs/cc-brief_fieldops-next-session.md` + tech_debt update (P2.4 → BLOCKED; P2.3 write-UI follow-up → RESOLVED).
...
- **`~/its` is on `main` (`5cc4336`)** — publish daemon loaded + healthy.
...
### On the horizon
- **Field-Ops portal (exec `5cc4336`, worktree `~/its-fieldops`, 2026-06-28):** P0 foundations + P1a/b/c forms + P2.1 schema + P2.2 read-views (A/B/C, PRs #308–#310) + P2.3 write-routes (6 slices, PRs #312–#317) + **write-UI phase complete (Slices 1–4, PRs #319–#322, 2026-06-28)**. P2.4 (Smartsheet/Box SoR mirror daemon) **BLOCKED** — no canonical Smartsheet access. Next unblocked work: **P3 Materials** (admin-UI-editable equipment/material catalog; brief in `docs/cc-brief_fieldops-next-session.md` once PR #323 merges). Canonical write pattern: `safety_portal/worker/fieldops_<domain>_write.ts` sub-module, `field-ops-write` capability gate (convenience, not External Send Gate). See auto-memory `project_fieldops-portal-program` + memory-archive §G42.
