...
- **`~/its` is on `main` (`763ad2b`)** — publish daemon loaded + healthy. (Operator: pull `git -C ~/its pull origin main` to complete cutover from the `51bb38d` base the session started from; `docs/tech_debt.md` local conflict resolves on pull.)
