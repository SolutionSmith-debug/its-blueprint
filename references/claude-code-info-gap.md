...
- **ITS — Progress Reporting program (updated 2026-07-01).** Strategic decisions locked. **Stage 0 COMPLETE. Stage 1 COMPLETE** (P1a #353 + P4-core #354 + P1b #356 + P1c #359). **Stage 2 (job-tracker→Smartsheet SoR pivot) DONE via P2.5** (PRs #383–#390, cut over LIVE 2026-07-01 — portal-owned canonical `JOB-######`, dual-sheet mirror daemon `fieldops_sync`, `sync_enabled=true`). **P2.6 Manager tier DEPLOYED LIVE** (PR #398, 2026-07-01 — 3rd role + `cap.crew.assign` + `personnel.current_job`). Stage 3: P2–P7 build progress workstream end-to-end (P2/P3/P4 done, P5 `progress_send` next). Track M: M2 (per-job Material List + bidirectional receive, after P7 + §51) + M3 (material incidents + photos, after M2). Personnel CRUD (#329) DONE. **Next queued: unified job-create flow** (bundle task-create + crew-assign + equipment-assign into the portal "New job" flow; crew converges on `personnel.current_job`, NOT `task_assignments` — a worker query change) — SPEC'd, not built: `~/.claude/plans/spec_unified-job-create-flow.md`. See memory-archive §G49.
- **Operator follow-ups pending (not CC):** (a) reload launchd plists (`install.sh`) to activate A2 RunAtLoad; (b) confirm Smartsheet per-plan sheet cap ($600 Pro vs $2,400 Business tier) then set `smartsheet.sheet_count_ceiling` in ITS_Config; (c) ~~§50/§51 doctrine bumps~~ **RATIFIED v19** (blueprint PR #50, exec #358, 2026-06-29); (d) meta-002 Tier-3 backup/escalation SLA before 20-job cutover.
...
- **`~/its` is on `main` (`551aa72`)** — publish daemon + portal_poll + fieldops_sync loaded + healthy. Stage-0 + Stage-1 COMPLETE; **P4 Slices 1–2 COMPLETE**; **P2.5 job-tracker→Smartsheet up-sync CUT OVER LIVE**; **P2.6 Manager tier DEPLOYED LIVE**. Stage-2 (job-tracker→Smartsheet SoR pivot) is DONE via P2.5 (superseding the original 6-slice framing — see §G49). P5 (`progress_send`) next in the Progress Reporting sequence; unified job-create flow SPEC'd (not built) as the next field-ops slice.
...
### On the horizon
- **Field-Ops portal + Progress-Reporting program (exec `551aa72`, 2026-07-01):** P0 foundations + P1a/b/c forms + P2.1 schema + P2.2 read-views (A/B/C, PRs #308–#310) + P2.3 write-routes (6 slices, PRs #312–#317) + write-UI phase complete (Slices 1–4, PRs #319–#322) + M1/P-A1/A2/UI-restyle (PRs #325–#328) + **Stage-0 COMPLETE** (#329/#344–#346/#349, 2026-06-29) + **Stage-1 COMPLETE** (#353/#354/#355/#356/#359, 2026-06-29/30) + **Stage-2 (P2.5 job-tracker→Smartsheet SoR pivot) DONE + CUT OVER LIVE** (#383–#390, 2026-07-01) + **P2.6 Manager tier DEPLOYED LIVE** (#398, 2026-07-01) + FF4/FF5 daemon hardening (#399/#400, 2026-07-01). P2.4 **BLOCKED** — no canonical Smartsheet access (superseded in spirit by the ITS-owned-SoR pivot; the original "wait for Evergreen Smartsheet visibility" framing no longer applies to the job-tracker itself). **ITS — Progress Reporting program:** strategic decisions locked; **P4 Slices 1–2 COMPLETE** (#375/#376); **P5 next** = `progress_reports/progress_send.py` (see tech_debt P5 forward-note: use `job.reports_contact_email` + `PROGRESS_ACTIVE_JOBS_CONFIG` — omitting the config silently routes to the safety contact). **Next field-ops slice: unified job-create flow** (SPEC'd, not built — `~/.claude/plans/spec_unified-job-create-flow.md`; crew-convergence on `personnel.current_job` is the one non-trivial worker change). Watchdog fast-follow: Check-C staleness + Check-I catch-up not yet wired to `progress_weekly_generate` slug (tech_debt entry OPEN; CLOSED for `fieldops_sync` via #397). Operator activations queued: (a) `git -C ~/its pull origin main`; (b) load `org.solutionsmith.its.progress-generate.plist` at progress cutover (not now); (c) §46 re-share safety approvers into `ITS — Progress Reporting` workspace (P5 send prerequisite). Op Stds v19 ratified (§50 + §51, blueprint PR #50 + exec #358). Canonical write pattern: `safety_portal/worker/fieldops_<domain>_write.ts` sub-module, `field-ops-write` capability gate (convenience, not External Send Gate). Blueprint: Progress-Reporting mission v1 draft in `workstreams/progress-reporting/mission.md` (see §G44). See memory-archive §G45 (Stage-0) + §G46 (Stage-1 + v19 + Stage-2 plan) + §G48 (P4 complete) + §G49 (P2.5 cutover + P2.6 + FF4/FF5) + auto-memory `project_fieldops-portal-program`.
