...
**Last refreshed:** 2026-06-29 (Stage-0 foundation complete: Personnel CRUD #329 + P0 heartbeat extraction #344 + A3 Box refresh-lock/keychain write-lock/50d freshness/Check P #345 + A6 weekly_generate hardening/compile_core #346 + A4 portal_poll backlog marker/Checks Q/R #349 — all four-part verified; watchdog letters P/Q/R active, O reserved. Forensic retrospective #330 + recurrence-guard meta-tests #342/#343 + doctrine-drift CI blocking #347 + hook guards #348 + Check Q CI-green detector #350 + CLAUDE.md narration rules #351 also landed. Exec HEAD `51bb38d`. Keychain set_secret TTY-trap live incident (A3 smoke) — corrupted ITS_BOX_REFRESH_TOKEN; recovered via argv reseed. See §G45.)
...
**Live incident (2026-06-29):** running the A3 Box OAuth smoke interactively called `_persist_tokens` → `set_secret` → TTY-trap → `ITS_BOX_REFRESH_TOKEN` corrupted → Box 401 on the next `portal_poll` cycle. Recovery: re-seeded via `security add-generic-password -U -a "$USER" -s ITS_BOX_REFRESH_TOKEN -w VALUE` (argv form). **Rule:** always run `setup_box_oauth.py` and any script that calls `set_secret` **headless** (e.g., `launchctl asuser` or a non-interactive subshell); never invoke them from an interactive terminal until the argv-form fix lands (standing task #8, `docs/tech_debt.md`). See also the 2026-06-08 trap above (manual bare-`-w` at the shell) and the auto-memory entry `feedback_keychain-security-cli-tty-stdin`.
...
- **ITS — Progress Reporting program — open (updated 2026-06-29).** Strategic decisions locked. **Stage 0 COMPLETE** (P-A1 + A2 + A3 + A4 + A6 + P0 all landed + four-part verified). **Stage 1 next unblocked:** P1a–P1c — parameterize `week_sheet` / `weekly_send` / `weekly_send_poll` with required config objects (no default = immediate `TypeError`). Stage 2: P2–P7 build progress workstream end-to-end. Track M: M2 (per-job Material List + bidirectional receive, after P7 + §51) + M3 (material incidents + photos, after M2). Personnel CRUD (#329) DONE.
- **Operator follow-ups pending (not CC):** (a) reload launchd plists (`install.sh`) to activate A2 RunAtLoad; (b) confirm Smartsheet per-plan sheet cap ($600 Pro vs $2,400 Business tier) then set `smartsheet.sheet_count_ceiling` in ITS_Config; (c) **§50 (code-actuation gate) + §51 (SoR-write / D1-as-writer to ITS-owned Smartsheet) doctrine bumps — proposed split, v18→v19, Seth-gated** (see §3 item 5 for full disambiguation); §51 gates P7+M2+M3 — initiate in parallel with Stage 0/1 work; (d) meta-002 Tier-3 backup/escalation SLA before 20-job cutover.
...
- **CLAUDE.md watchdog count stale** — says "6 of 7 checks operational"; actual active count is now **14 checks** (A,B,C,D,F,G,I,J,K,L,M,P,Q,R), only E deferred. Watchdog letter ledger: P=Box freshness (50d), Q=portal_poll fetch-outage, R=portal_poll unfiled-backlog, O=reserved (A5 row-cap). Surfaced by PR #156; count further advanced by PRs #345/#349 (2026-06-29). Fix: one-line CLAUDE.md update.
- **`keychain.set_secret` TTY-trap fix pending** (task #8) — interactive Python sessions inherit the controlling TTY; `security add-generic-password -w` reads from `/dev/tty`, ignores piped stdin, silently corrupts the secret. See §5 new trap + `docs/tech_debt.md`. HIGH, secrets/auth.
...
- **`~/its` is on `main` (`51bb38d`)** — publish daemon loaded + healthy.
...
### On the horizon
- **Field-Ops portal + Progress-Reporting program (exec `51bb38d`, 2026-06-29):** P0 foundations + P1a/b/c forms + P2.1 schema + P2.2 read-views (A/B/C, PRs #308–#310) + P2.3 write-routes (6 slices, PRs #312–#317) + write-UI phase complete (Slices 1–4, PRs #319–#322) + M1/P-A1/A2/UI-restyle (PRs #325–#328) + **Personnel CRUD #329 + P0 #344 + A3 #345 + A6 #346 + A4 #349 (Stage-0 COMPLETE, 2026-06-29)**. P2.4 **BLOCKED** — no canonical Smartsheet access. **ITS — Progress Reporting program open:** strategic decisions locked (weekly sheets [monthly reverted 2026-06-29], ITS-owned SoR, parameterize-not-clone, Tier-A gate front-loaded); **Stage 0 COMPLETE; Stage 1 next** (P1a–P1c parameterize `week_sheet`/`weekly_send`/`weekly_send_poll`); Stage 2 build progress workstream end-to-end. Canonical write pattern: `safety_portal/worker/fieldops_<domain>_write.ts` sub-module, `field-ops-write` capability gate (convenience, not External Send Gate). Operator: confirm Smartsheet tier cap + reload plists (A2 RunAtLoad) + initiate §50/§51 doctrine bumps (proposed split — see §3 item 5) + keychain TTY-trap fix (task #8). Blueprint: Progress-Reporting mission v1 draft in `workstreams/progress-reporting/mission.md` (see §G44). See memory-archive §G45 (Stage-0 foundation + keychain TTY-trap) + §G43 (previous session) + auto-memory `project_fieldops-portal-program`.
