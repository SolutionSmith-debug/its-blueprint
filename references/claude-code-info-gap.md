...
- **ITS — Progress Reporting program (updated 2026-06-30).** Strategic decisions locked. **Stage 0 COMPLETE.** **Stage 1 COMPLETE** (P1a #353 + P4-core #354 + P1b #356 + P1c #359 — all four-part verified, exec HEAD `763ad2b`). **Stage 2 re-planned as job-tracker→Smartsheet SoR pivot:** two physical Active-Jobs sheets (field-ops + progress-reporting) + Portal Job Key bridge + dual-sheet mirror daemon; 6 slices; folds into the roadmap as a P2 topology revision + new P2.5. Plan: `~/.claude/plans/ok-we-are-going-scalable-flamingo.md`. Stage 3: P2–P7 build progress workstream end-to-end. Track M: M2 (per-job Material List + bidirectional receive, after P7 + §51) + M3 (material incidents + photos, after M2). Personnel CRUD (#329) DONE.
...
- **`~/its` is on `main` (`b91ed8d`)** — publish daemon loaded + healthy. Stage-0 + Stage-1 COMPLETE; Stage-2 (job-tracker→Smartsheet SoR pivot) planned, not yet started.
...
### On the horizon
- **Field-Ops portal + Progress-Reporting program (exec `b91ed8d`, 2026-06-30):** P0 foundations + P1a/b/c forms + P2.1 schema + P2.2 read-views (A/B/C, PRs #308–#310) + P2.3 write-routes (6 slices, PRs #312–#317) + write-UI phase complete (Slices 1–4, PRs #319–#322) + M1/P-A1/A2/UI-restyle (PRs #325–#328) + **Stage-0 COMPLETE** (#329/#344–#346/#349, 2026-06-29) + **Stage-1 COMPLETE** (#353/#354/#355/#356/#359, 2026-06-29/30). P2.4 **BLOCKED** — no canonical Smartsheet access. **ITS — Progress Reporting program:** strategic decisions locked (weekly sheets, ITS-owned SoR, parameterize-not-clone, Tier-A gate front-loaded); Stages 0+1 COMPLETE; **Stage 2 next = job-tracker→Smartsheet SoR pivot** (two Active-Jobs sheets + Portal Job Key bridge + dual-sheet mirror daemon, 6 slices; plan `~/.claude/plans/ok-we-are-going-scalable-flamingo.md`); Stage 3 build progress workstream end-to-end. Op Stds v19 ratified (§50 + §51, blueprint PR #50 + exec #358). Canonical write pattern: `safety_portal/worker/fieldops_<domain>_write.ts` sub-module, `field-ops-write` capability gate (convenience, not External Send Gate). Operator: confirm Smartsheet tier cap + reload plists (A2 RunAtLoad). Blueprint: Progress-Reporting mission v1 draft in `workstreams/progress-reporting/mission.md` (see §G44). See memory-archive §G45 (Stage-0 + keychain TTY-trap) + §G46 (Stage-1 + v19 + Stage-2 plan) + auto-memory `project_fieldops-portal-program`.
