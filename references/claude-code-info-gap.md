...
**Last refreshed:** 2026-06-09 (evening — Form Editor UX fixes + draft caching + live SPA deploy. PRs #249/#250 merged to main (`8c1600d`). Response-scale now uses per-option OptionsEditor (no trailing-comma erasure). explainPublish maps 401 idle-timeout and bad_request; fallback names code+status (no more silent "Publish was rejected"). Per-account localStorage draft cache (key `its-portal-draft:v1:<username>`): autosave, auto-restore on mount, Discard/publish clear it, Cancel keeps it. SPA deployed live to `safety.evergreenmirror.com` (CF version `71428941-53e5-46fc-8d8b-3570232d58d7`). `~/its` on `main` (`8c1600d`). Publish daemon loaded + live. Next: edge-case matrix, load compile_now_poll, Evergreen production cutover.)
...
- **Safety Portal Phase-2 Form Manager — BUILT + publish-pipeline hardened + Form Editor UX polished (2026-06-09, exec HEAD `8c1600d`).** See "Recently landed" above + memory-archive §G30 + §G31 + §G32. Deferred: rollback UI picker (frontend only — backend done), S1 per-item scale/comment authoring, integration test harness for the daemon's git/gh/wrangler subprocess chain, portal admin Retire UI gate (shows for already-retired forms; backend rejects clean), `README.md:111` doc-drift (weekly-send idempotency key), `form_archive_out/` temp-dir cleanup, draft-cache one-slot-per-account limitation.
- **`~/its` is on `main` (`8c1600d`)** — publish daemon loaded + healthy.
...
- **WSR_human_review live columns retyped DATE → ABSTRACT_DATETIME** (mirror, 2026-06-09) — `8c1600d` code writes naive Pacific timestamps; PRODUCTION sheet built from updated builder gets ABSTRACT_DATETIME from creation.

### On the horizon
- **Safety Portal** — **Phase-2 Form Manager FULLY BUILT + publish pipeline hardened + Form Editor UX polished (2026-06-09, exec HEAD `8c1600d`; mirror SPA live).** Phase-1 mirror-activated + live-validated (2026-06-08). Publish daemon IS loaded (live). `portal_poll` + `weekly_send_poll` live + healthy. `ZZ Portal Proof` (JOB-000008) Active. Next: (1) full edge-case matrix (bad-HMAC, dup-UUID, malformed payload, inactive-job, amend-supersede, empty-week, Friday-skip, empty-TO HELD, F22 non-member); (2) load `compile_now_poll` daemon; (3) activate Orphaned Reports (flip `SHEET_ORPHANED_REPORTS`); (4) Evergreen production cutover. Deferred form-editor items: rollback UI picker (frontend only), S1 per-item scale/comment. See memory-archive §G27 (activation) + §G30 (Phase-2 build) + §G31 (pipeline hardening + WSR timestamps) + §G32 (Form Editor UX + draft caching).
